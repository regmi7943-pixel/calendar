...
function generateSVG(type: CalendarType, data: any, theme: 'dark' | 'light', specs: DeviceSpecs): string {
...
export async function GET(request: NextRequest) {
    ...
    const device = (searchParams.get('device') as DeviceId) || 'iphone_14_15_promax';
    const dob = searchParams.get('dob');
    const goalDate = searchParams.get('goalDate');
    const theme = searchParams.get('theme') === 'light' ? 'light' : 'dark';

    const specs = DEVICE_CONFIGS[device] || DEVICE_CONFIGS.iphone_14_15_promax;
    ...
    try {
        ...
        const svg = generateSVG(type, data, theme, specs);
